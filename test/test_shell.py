...
class TestShell(unittest.TestCase):
    ...
    def test_to_string_call(self):
        ...
        self.assertEqual(object_to_string, "Call(Argument(['echo']), [Argument(['hello'])])")
    def test_to_string_sequence(self):
        object_to_string = str(Seq(Call([Argument(['echo']), Argument(['a'])]), Call([Argument(['echo']), Argument(['b'])])))
        self.assertEqual(object_to_string, "Seq(Call(Argument(['echo']), [Argument(['a'])]), Call(Argument(['echo']), [Argument(['b'])]))")
    ...
    def test_visitor(self):
        instance = Visitor()
        self.assertEqual(True, instance.visit_argument(None) == instance.visit_back_quoted(None) == instance.visit_call(None) == instance.visit_double_quoted(None) == instance.visit_pattern(None) == instance.visit_pipe(None) == instance.visit_seq(None) == instance.visit_single_quoted(None) == instance.visit_redirection(None))
    def test_grammar_visitor(self):
        instance = ShellGrammarVisitor()
        self.assertEqual(True, instance.visitCall(None) == instance.visitArgument(None) == instance.visitCommand(None) == instance.visitPipe(None) == instance.visitQuoted(None) == instance.visitRedirection(None))
