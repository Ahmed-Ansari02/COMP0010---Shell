...
from Applications import Pattern, APPLICATIONS
...
class Evaluator(Visitor):
    ...
    def visit_argument(self, argument):
        to_return = "".join(
            [element.accept(self)
                if (type(element) != str and type(element) != io.StringIO)
                else element for element in argument.argument_list])
        return to_return
    ...
    def visit_double_quoted(self, quoted):
        ...
        return ''.join(
            [token
                ...
                else evaluate(convert(token[1:-1])) for token in tokens])
